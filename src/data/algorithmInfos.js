const algorithmInfos = {
	bubble_sort: {
		...
		recurrence_relation: {
			best: "T(n) = O(n) - No swaps needed, single pass",
			average: "T(n) = T(n-1) + O(n) = O(n²)",
	...
	insertion_sort: {
		...
		space_complexity: ['O(1)', 'green-800'],
		recurrence_relation: {
			best: "T(n) = O(n) - Each element in correct position",
			average: "T(n) = T(n-1) + O(n) = O(n²)",
			worst: "T(n) = T(n-1) + O(n) = O(n²)"
		},
		description: `
	...
	merge_sort: {
		...
		recurrence_relation: {
			...
			worst: "T(n) = 2T(n/2) + O(n) = O(n log n)"
		},
		description: `
	...
	quick_sort: {
		...
		recurrence_relation: {
			best: "T(n) = 2T(n/2) + O(n) = O(n log n)",
			average: "T(n) = T(n-1) + T(0) + O(n) = O(n log n)",
			worst: "T(n) = T(n-1) + T(0) + O(n) = O(n²)"
		},
		description: `
	...
	heap_sort: {
		...
		space_complexity: ['O(1)', 'green-800'],
		recurrence_relation: {
			best: "T(n) = O(n) + n×O(log n) = O(n log n)",
			average: "T(n) = O(n) + n×O(log n) = O(n log n)",
			worst: "T(n) = O(n) + n×O(log n) = O(n log n)"
		},
		description: `
			Heap Sort is a comparison-based sorting algorithm that uses a binary heap data structure to sort elements. It is one of the most efficient sorting algorithms with guaranteed O(n log n) time complexity in all cases.

			The algorithm works in two main phases: First, it builds a max-heap from the input array, where the largest element is at the root. Then, it repeatedly extracts the maximum element from the heap and places it at the end of the array, reducing the heap size by one each time. This process continues until the heap is empty and the array is sorted.

			Heap Sort is an in-place algorithm with O(1) space complexity, making it memory efficient. It is also stable in terms of time complexity - it always takes O(n log n) time regardless of the input data distribution. However, it is not a stable sort (doesn't preserve the relative order of equal elements) and has poor cache performance compared to other algorithms like Quick Sort.
		`
	},
	radix_sort: {
		name: "Radix Sort",
		...
		recurrence_relation: {
			best: "T(n) = k×O(n) = O(nk) where k = digits",
			average: "T(n) = k×O(n) = O(nk) where k = digits",
			worst: "T(n) = k×O(n) = O(nk) where k = digits"
		},
		description: `
	...
	bucket_sort: {
		name: "Bucket Sort",
		time_complexity: {
			best: ['O(n+k)', 'green-800'],
			average: ['O(n+k)', 'green-800'],
			worst: ['O(n^2)', 'red-800']
		},
		space_complexity: ['O(n+k)', 'yellow-600'],
		recurrence_relation: {
			best: "T(n) = O(n) + k×O(n/k) = O(n+k)",
			average: "T(n) = O(n) + k×O(n/k) = O(n+k)",
			worst: "T(n) = O(n) + k×O(n²/k²) = O(n²)"
		},
		description: `
			Bucket Sort is a distribution sorting algorithm that works by dividing the array into a number of buckets, sorting each bucket individually, and then concatenating the buckets to produce the final sorted array.

			The algorithm works by first determining the range of values in the input array and then creating a number of buckets equal to the range divided by the number of elements. Each element is then placed into its appropriate bucket based on its value. The buckets are then sorted individually using another sorting algorithm (often insertion sort), and finally, the sorted buckets are concatenated to produce the final sorted array.

			Bucket Sort is most effective when the input data is uniformly distributed across the range. In this case, it has an average time complexity of O(n+k), where n is the number of elements and k is the number of buckets. However, if the data is not uniformly distributed, some buckets may contain many more elements than others, leading to a worst-case time complexity of O(n^2). The space complexity is O(n+k) due to the additional space required for the buckets.
		`
	},
}
